                        ...
                        if let Some(content) = content {
                            if content.len() == 8 {
                                ...
                                if width > 0.0 && height > 0.0 {
                                    // Randomize location within element's bounding box
                                    let x = rng.gen_range(x1..x2);
                                    let y = rng.gen_range(y1..y2);

                                    let node_for_location = match self
                                        .send(
                                    ...
                                    log::debug!(
                                        "asd: {:?} == {:?}",
                                        node_for_location.get("nodeId"),
                                        node_id_value,
                                    );

                                    if node_for_location.get("nodeId")
                                        != Some(&json!(node_id_value))
                                    {
                                        // Check if element is disabled or aria-disabled
                                        let is_disabled = match self
                                            .send(
                                            ...
                                            .await
                                        {
                                            Ok(attributes_result) => {
                                                let result = attributes_result.get_result();
                                                let attributes =
                                                    match result["attributes"].as_array() {
                                                        Some(attrs) => attrs,
                                                        None => {
                                                            sleep(Duration::from_secs(1)).await;
                                                            continue;

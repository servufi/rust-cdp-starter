...
async fn main() -> Result<()> {
    ...
    let headless1_params = Some(json!({
        ...
        "profile_name": "profile0", // new profile name = new browser window, default: "profile1" (sanitized to: a-z0-9 spaces replaced with _ and to lowercase)
        "args": [ // Optional chrome launch args
            "--headless=new", // this incognito argument isn't supported and will be ignored. (Didn't find nice way to access initial browser context target)
    ...
    let browser2_params = Some(json!({
        "profile_name": "profile2", // new profile name = new browser window, default: "profile1" (sanitized to: a-z0-9 spaces replaced with _ and to lowercase)
        "args": [ // Optional chrome launch args
